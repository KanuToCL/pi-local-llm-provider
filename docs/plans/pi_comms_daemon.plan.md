...
| V5-H | Product vision (Sergio 2026-05-03) | **Guided first-run setup via `/setup-comms`** — instead of manually editing `.env`, the user sends `/setup-comms` to the bot (or runs it in a terminal) and is walked through configuration interactively: bot token, allowed user ID, Studio URL + API key, model selection. Daemon writes the `.env` itself and restarts. Stretch: a web-based setup wizard served locally on first boot (before Telegram is configured) so the user never needs to touch a text file. This closes the "barrier to entry" gap for non-technical users and makes re-configuration (e.g. rotating keys, switching models) safe and guided. | New Phase 6 |
| V5-G | Product vision (Sergio 2026-05-03) | **Cloud escalation via `/consult`** — **user-only trigger, never agent-autonomous**. User sends `/consult claude` (or `/consult gpt4`, etc.) from Telegram when they judge pi is stuck; daemon packages the current session state (task, last N errors, relevant file excerpts) and calls the cloud API with a guidance request; cloud response is streamed back to Telegram and optionally injected into pi's context as a nudge. Pi cannot invoke this itself — no `consult_cloud()` tool, no automatic escalation on N-failed-attempts. The decision to send data to a third party always belongs to the user. Design requirements: `CLOUD_CONSULT_ENABLED=true` opt-in env flag; pluggable provider (Anthropic \| OpenAI \| configurable base_url); configurable state-packaging depth (last N turns, full thread, or task-only summary); user sees a preview of what will be sent before the call goes out (confirm gate, R-cloud-1). | New Phase 6 |
